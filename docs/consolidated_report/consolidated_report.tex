...
\subsection{Engineering Tolerance \& Robust Score Formulation}
\label{sec:robust_score}
To evaluate the operational resilience of Pareto-optimal operating points against machine instabilities, sensitivity studies are conducted by sampling perturbations under realistic engineering tolerances ($\pm 0.1^\circ$ RF phase jitter and $\pm 0.1\%$ magnet field gradient fluctuations).

Candidate robustness is quantified using a composite metric named the \textit{Robust Score} ($S_{\text{robust}}$):
\begin{equation}
    S_{\text{robust}} = \frac{P_{\text{feas}}}{\max\left(1.0, \, \frac{\langle \varepsilon_{n,x} \rangle}{\varepsilon_{n,x}^{\text{nominal}}}\right)}
\end{equation}
where:
\begin{itemize}
    \item $P_{\text{feas}} = \frac{N_{\text{feasible}}}{N_{\text{total}}}$ is the Probability of Feasibility, representing the fraction of perturbed evaluations satisfying all 8 beam quality constraints.
    \item $\varepsilon_{n,x}^{\text{nominal}}$ is the nominal horizontal emittance of the unperturbed Pareto candidate.
    \item $\langle \varepsilon_{n,x} \rangle$ is the mean horizontal emittance measured across valid perturbed evaluations.
    \item The $\max(1.0, \dots)$ cap ensures that incidental emittance reductions under jitter do not unphysically inflate the score.
\end{itemize}

Operating points with $S_{\text{robust}} \ge 0.80$ are classified as robust operating candidate points suitable for physical machine implementation, while candidates with $S_{\text{robust}} < 0.80$ are identified as fragile solutions operating too close to constraint boundaries.
...
\bibitem{ICABU2025}
C.S.~Park \textit{et al.}, ``Scalarized and Multi-Objective Bayesian Optimization for a 200 MeV Electron Injector Linac,'' in \textit{Proc. ICABU 2025}, 2025.
