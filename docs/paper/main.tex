...
\author{
    \textbf{Accelerator Physics \& Optimization Group}\\
    Pohang Accelerator Laboratory (PAL) / Department of Physics\\
    \texttt{contact@pal.re.kr}
}
...
Independent Gaussian Process (GP) surrogates with Mat\'{e}rn-5/2 kernels model each objective. Candidate batches of size $q=4$ are selected by optimizing the $q\text{LogNEHVI}$ acquisition function:
\begin{equation}
...
Controlled campaigns were executed comparing Phase 2 (Unconstrained MOBO) and Phase 3 (Constrained MOBO) under identical random seeds ($N_{\text{init}}=16$, $q=4$, 6 iterations, total budget 40). A common fixed reporting reference point was set at $\mathbf{r}_{\text{rep}} = [-6.65\times 10^{-5}, -1.07\times 10^{-4}, -3.37\times 10^6]$ in model space.
