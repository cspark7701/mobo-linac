...
\subsection{Surrogate Modeling and Feasibility Evaluation}
\label{sec:surrogates}
Independent Gaussian Process (GP) surrogates with Mat\'{e}rn-5/2 kernels ($\nu = 2.5$) and 6D Automatic Relevance Determination (ARD) model each objective independently using \texttt{ModelListGP}. To handle objective dimensions differing by orders of magnitude ($\mu\text{m}\cdot\text{rad}$ vs $\text{MeV}$), we implement relative observation noise variance scaling $\sigma_{\text{obs},m}^2 = \max(\eta \cdot \operatorname{Var}(Y_m), \sigma_{\text{floor}}^2)$ with $\eta = 10^{-6}$ and $\sigma_{\text{floor}}^2 = 10^{-24}$, preventing numerical over-smoothing while accommodating deterministic ASTRA simulations. In constrained MOBO (Phase 3), additional GP surrogates model the 7 diagnostic constraint channels ($[\sigma_x, \sigma_y, \sigma_{x'}, \sigma_{y'}, \sigma_z, E_{\text{kin}}, \text{transmission}]$) with exact analytical multi-channel Normal CDF feasibility evaluation:
\begin{equation}
...
\subsection{Acquisition Function and Fault-Tolerant Optimization}
...
\begin{equation}
    ...
    \right]
\end{equation}
where $\tilde{\mathbf{y}}_i$ are joint posterior samples from the GP models evaluated at the proposed $q$ candidates $\mathbf{X}$, $\mathcal{P}$ is the current Pareto archive in model space, and $\mathbf{r}_{\text{acq}}$ is the acquisition reference point. In Phase 3, the acquisition is additionally weighted by the analytical Probability of Feasibility $P_{\text{feas}}(\mathbf{x})$ \cite{Daulton2021qNEHVI}. To protect against optimizer singularities during long campaigns, candidate generation features adaptive multi-restart L-BFGS retries with quasi-random Sobol fallback and atomic POSIX checkpoint state persistence.
...
Controlled campaigns were executed systematically comparing Phase~1 (Scalarized BO), Phase~2 (Unconstrained MOBO), and Phase~3 (Constrained MOBO) under standardized evaluation conditions with Sobol-sampled initialization points ($N_{\text{init}} = 16$), candidate proposal batches, and identical lattice geometry. All hypervolumes reported below are computed against the fixed physical reporting reference point $\mathbf{r}_{\text{rep}}$ (Section~\ref{sec:ref_point}).

A campaign summary comparing key outcome metrics across the three optimization phases is provided in Table~\ref{tab:campaign_summary}.

\subsection{Phase 1: Scalarized Bayesian Optimization Performance}
\label{sec:phase1_results}
Phase~1 executes scalarized Bayesian Optimization combining normalized transverse emittances and energy spread into a composite merit function ($w_x \tilde{\varepsilon}_{n,x} + w_y \tilde{\varepsilon}_{n,y} + w_E \tilde{\sigma}_E$). To systematically evaluate operating trade-offs, five representative linac archetypes (Option 2) are formulated: balanced linac operation ($[1/3, 1/3, 1/3]$), high-brightness FEL mode ($[0.45, 0.45, 0.10]$), low energy spread mode ($[0.10, 0.10, 0.80]$), and horizontal/vertical asymmetric optics ($[0.60, 0.20, 0.20]$ and $[0.20, 0.60, 0.20]$), as summarized in Table~\ref{tab:phase1_cases}.

All evaluations converged numerically without execution failures ($100\%$ simulation validity). As reported in Table~\ref{tab:campaign_summary}, the scalarized search achieved minimum single-channel metrics of $\varepsilon_{n,x} = 2.9102~\mu\text{m}\cdot\text{rad}$, $\varepsilon_{n,y} = 4.9287~\mu\text{m}\cdot\text{rad}$, and $\sigma_E = 0.2769~\text{MeV}$. Tracking all valid points against the standardized reference point $\mathbf{r}_{\text{rep}}$ yielded an unconstrained hypervolume of $0.019588$. However, because scalarized optimization does not explicitly model or enforce joint beam quality constraints during acquisition selection, beam size ($\sigma_x \le 1.0\text{ mm}$, $\sigma_y \le 1.0\text{ mm}$) and bunch length ($\sigma_z \le 1.0\text{ mm}$) constraint violations severely limit the discovery of fully compliant beams ($1.1\%$ feasible evaluations in the balanced baseline), yielding a final feasible hypervolume of $0.019029$ with only 2 feasible Pareto points. In contrast, Phase~2 discovered 27 feasible beams and Phase~3 discovered 75 feasible beams (42.6\% feasibility fraction), demonstrating the decisive operational advantage of true constraint-aware Multi-Objective Bayesian Optimization.

\input{phase1_cases_table}
...
\begin{figure}[H]
    ...
    \caption{Feasible hypervolume progress over cumulative ASTRA evaluations comparing Phase~1 (Scalarized), Phase~2 (Unconstrained), and Phase~3 (Constrained) MOBO. Final hypervolume values are derived from \texttt{hypervolume.csv} in each campaign run directory.}
    \label{fig:hypervolume}
\end{figure}

As shown in Figure~\ref{fig:hypervolume}, all campaigns achieve progressive hypervolume growth, with Phase~3 demonstrating the highest final feasible hypervolume. Final feasible hypervolume values and feasibility statistics are reported in Table~\ref{tab:campaign_summary}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/pareto_front_comparison.png}
    \caption{Two-dimensional projections of the physical objective space comparing Phase~1, Phase~2, and Phase~3 Pareto fronts (sourced from \texttt{pareto.csv}).}
    \label{fig:pareto}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/feasible_fraction.png}
    \caption{Feasible beam fraction as a function of cumulative ASTRA evaluations comparing Phase~1 (Scalarized), Phase~2 (Unconstrained), and Phase~3 (Constrained) MOBO.}
    \label{fig:feasible_fraction}
\end{figure}

Figure~\ref{fig:feasible_fraction} plots the evolution of the feasible beam fraction. While Phase~1 remains near $1.1\%$ and Phase~2 plateaus near $15.3\%$, Phase~3 rapidly guides search evaluations into the feasible operational space, reaching $42.6\%$ overall feasibility.
