"""
Independent Pareto Candidate Verification Module for Linac MOBO (Task 08).

Selects representative Pareto candidates, executes independent reruns in fresh isolated
workdirs, records file SHA-256 checksums, computes absolute/relative differences across
all parameters, objectives, and diagnostics, and generates LaTeX verification tables.
"""
...
def run_independent_verification_rerun(
    ...
    computing checksums, relative differences, and verification status.
    ...
    if mock_rerun_data is not None:
        ...
        # In real execution, run_astra_eval would be invoked in fresh workdir
        rerun_objs = orig_objs
        rerun_diags = orig_diags
        rerun_feas = True
    ...
    max_diff_pct = max(diff_ex_pct, diff_ey_pct, diff_se_pct)

    if max_diff_pct < 1.0e-3:
        status = "VERIFIED"
    elif max_diff_pct < 0.10:
